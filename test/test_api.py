...
def test_list_of_profiles_revalidation(test_client: FlaskClient) -> None:
    ...
    response = test_client.get(
        "/profiles", headers={"If-None-Match": response.headers.get("ETag")}
    )
...
def test_list_of_profiles_only_contains_snippets(
    test_client: FlaskClient, commit: Callable[[], None]
) -> None:
...
def test_list_of_profiles_in_ascending_order(
    test_client: FlaskClient, commit: Callable[[], None]
) -> None:
    for number in range(1, 31):
        number = str(number).zfill(2)
...
def test_list_of_profiles_in_pages(
    test_client: FlaskClient, commit: Callable[[], None]
) -> None:
...
def test_get_profile_revalidation(
    test_client: FlaskClient, commit: Callable[[], None]
) -> None:
    profile = Profile("a1b2c3d4", Name("Foo Bar"), "0000-0002-1825-0097")
    ...
    response = test_client.get(
        "/profiles/a1b2c3d4", headers={"If-None-Match": response.headers.get("ETag")}
    )
...
def test_get_profile_response_contains_email_addresses(
    test_client: FlaskClient, commit: Callable[[], None]
) -> None:
    profile = Profile("a1b2c3d4", Name("Foo Bar"), "0000-0002-1825-0097")
...
def test_get_profile_response_contains_affiliations(
    test_client: FlaskClient, yesterday, commit: Callable[[], None]
) -> None:
    address = Address(country=countries.get("gb"), city="City", region="Region")
    affiliation = Affiliation(
        "1", address=address, organisation="Org", department="Dep", starts=yesterday
    )
...
def test_it_does_not_return_restricted_affiliations(
    test_client: FlaskClient, yesterday, commit: Callable[[], None]
) -> None:
    address = Address(country=countries.get("gb"), city="City", region="Region")
    affiliation = Affiliation(
...
def test_it_does_return_restricted_data_when_authenticated(
    test_client: FlaskClient, yesterday: Date, commit: Callable[[], None]
) -> None:
...
def test_it_does_not_return_null_values_in_response(
    ...
    address = Address(
        country=countries.get("gb"), city="City", region=None
    )  # produces null value when serialized
    affiliation = Affiliation(
        "2",
        address=address,
        organisation="Org",
