...
def test_authorizing_requires_a_valid_redirect_uri(test_client: FlaskClient) -> None:
    response = test_client.get(
        "/oauth2/authorize",
        query_string={"client_id": "client_id", "redirect_uri": "http://www.evil.com/"},
    )
...
def test_authorizing_requires_a_response_type(test_client: FlaskClient) -> None:
    response = test_client.get(
        "/oauth2/authorize", query_string={"client_id": "client_id"}
    )

    assert response.status_code == 302
    assert response.headers[
        "Location"
    ] == "http://www.example.com/client/redirect?" + urlencode(
...
def test_it_redirects_when_authorizing(test_client: FlaskClient) -> None:
    ...
    assert response.headers[
        "Location"
    ] == "http://www.example.com/oauth/authorize?" + urlencode(
        {
...
def test_it_redirects_with_the_original_state_when_authorizing(
...
def test_it_requires_a_json_state_when_checking(test_client: FlaskClient) -> None:
    response = test_client.get(
        "/oauth2/check", query_string={"code": 1234, "state": "foo"}
    )
...
def test_it_requires_redirect_uri_in_state_when_checking(
    test_client: FlaskClient,
) -> None:
    response = test_client.get(
        "/oauth2/check",
...
def test_it_redirects_when_checking(test_client: FlaskClient) -> None:
    ...
    assert response.headers[
        "Location"
    ] == "http://www.example.com/client/redirect?" + urlencode({"code": 1234}, True)


def test_it_redirects_with_the_original_state_when_checking(
    test_client: FlaskClient,
) -> None:
    response = test_client.get(
        "/oauth2/check",
...
def test_it_exchanges(
    generate_random_string: MagicMock, test_client: FlaskClient
) -> None:
...
def test_it_still_returns_200_when_failing_to_read_orcid_data(
    test_client: FlaskClient,
) -> None:
    with requests_mock.Mocker() as mocker:
        mocker.post(
...
def test_it_updates_the_access_token_when_exchanging(test_client: FlaskClient) -> None:
    original_orcid_token = OrcidToken(
        "0000-0002-1825-0097", "old-access-token", expires_at(1234)
    )
...
def test_it_requires_a_bearer_token_type_when_exchanging(
    test_client: FlaskClient,
) -> None:
    with requests_mock.Mocker() as mocker:
        mocker.post(
