...
def test_it_may_have_a_department():
    has = Affiliation(
        "1",
        Address(countries.get("gb"), "City"),
        "Organisation",
        Date(2017),
        department="Department",
    )
    has_not = Affiliation(
        "2", Address(countries.get("gb"), "City"), "Organisation", Date(2017)
    )
...
def test_it_has_a_start_date(yesterday):
    affiliation = Affiliation(
        "1", Address(countries.get("gb"), "City"), "Organisation", yesterday
    )
...
def test_it_can_detect_if_current_with_ends_date(tomorrow, yesterday):
    address = Address(countries.get("gb"), "City")
    affiliation = Affiliation(
        "1", address=address, organisation="Org", starts=yesterday, ends=tomorrow
    )
...
def test_it_can_detect_if_not_current_with_past_starts_date_and_past_ends_date(
    ...
    address = Address(countries.get("gb"), "City")
    affiliation = Affiliation(
        "1", address=address, organisation="Org", starts=yesterday, ends=yesterday
    )
