...
def test_it_sets_a_webhook_when_a_profile_is_inserted(
    profile: Profile,
    ...
    webhook_maintainer = maintain_orcid_webhook(
        orcid_config, mock_orcid_client, url_safe_serializer
    )
    ...
    assert mock_orcid_client.set_webhook.call_args[0][
        1
    ] == "http://localhost/orcid-webhook/{}".format(
        url_safe_serializer.dumps("0000-0002-1825-0097")
    )
...
def test_it_will_remove_the_webhook_when_a_profile_is_deleted(
    profile: Profile,
    orcid_config: Dict[str, str],
    mock_orcid_client: MagicMock,
    session: scoped_session,
