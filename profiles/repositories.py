...
class Profiles(CanBeCleared, collections.Sized):
    ...
    @abstractmethod
    def list(
        self, limit: int = None, offset: int = 0, desc: bool = False
    ) -> List[Profile]:
        raise NotImplementedError
...
class SQLAlchemyOrcidTokens(OrcidTokens):
    ...
    def remove(self, orcid: str) -> None:
        try:
            ...
            LOGGER.info(
                "Unable to remove ORCID token for ORCID %s. Token not found", orcid
            )


class SQLAlchemyProfiles(Profiles):
    def __init__(
        self, db: SQLAlchemy, next_id_generator: Callable[[], str] = None
    ) -> None:
        if next_id_generator is None:

            def generate_id():
                return generate_random_string(
                    ID_LENGTH, string.ascii_lowercase + string.digits
                )
    ...
    def add(self, profile: Profile) -> Profile:
        ...
        try:
            ...
            if "orcid" in message and profile.orcid:
                ...
                log_msg += "Profile with email address {} already exists".format(
                    email_addresses
                )
                LOGGER.info(msg=log_msg)
    ...
    def get_by_email_address(self, *email_addresses: str) -> Profile:
        ...
        try:
            return (
                self.db.session.query(Profile)
                .join(EmailAddress)
                .filter(EmailAddress.email.in_(email_addresses))
                .one()
            )
        except NoResultFound as exception:
    ...
    def next_id(self) -> str:
        ...
        if (
            self.db.session.query(Profile.id).filter_by(id=profile_id).scalar()
            is not None
        ):
    ...
    def list(
        self, limit: int = None, offset: int = 0, desc: bool = True
    ) -> List[Profile]:
