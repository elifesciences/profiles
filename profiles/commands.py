...
def update_profile_from_orcid_record(profile: Profile, orcid_record: dict) -> None:
    LOGGER.info(
        "Updating profile %s with ORCID record %s", profile.id, orcid_record.get("path")
    )
...
def extract_email_addresses(
    orcid_record: dict, only_verified: bool = True
) -> List[dict]:
...
def _update_affiliations_from_orcid_record(
    profile: Profile, orcid_record: dict
) -> None:
    orcid_affiliations = (
        jmespath.search(
            '"activities-summary".employments."employment-summary"[*]', orcid_record
        )
        or []
    )
...
def _update_email_addresses_from_orcid_record(
    profile: Profile, orcid_record: dict
) -> None:
    ...
    for index, email_dict in enumerate(orcid_email_dicts):
        ...
        if email_profile.id != profile.id:
            message = (
                "Profile %s is trying to add email address %s but this "
