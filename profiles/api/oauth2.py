...
def create_blueprint(
    ...
    def _check() -> Response:
        ...
        return redirect(
            "{}?{}".format(state["redirect_uri"], urlencode(query, True)), code=302
        )
    ...
    def _token() -> Response:
        if "client_id" not in request.form:
            LOGGER.error(
                "InvalidClient: /token POST request received without client_id specified"
            )
            raise InvalidClient

        LOGGER.info(
            "/token POST request received from %s", request.form.get("client_id")
        )
        ...
        try:
            ...
            LOGGER.error(
                "InvalidClient: %s not in list of known clients",
        ...
        if request.form.get("client_secret") != client.client_secret:
            ...
            LOGGER.error(
                "InvalidRequest: redirect_uri not in client.redirect_uris - %s",
                redirect_uri,
            )
        ...
        response = requests.post(
            url=orcid["token_uri"], data=data, headers={"Accept": "application/json"}
        )
        ...
        json_data = {
            key: json_data[key]
            for key in ["access_token", "expires_in", "name", "orcid", "token_type"]
        }

        if json_data.get("token_type").lower() != "bearer":
            raise ValueError(
                "Got token_type {}, expected Bearer".format(json_data.get("token_type"))
            )
    ...
    def _find_profile(token_data: dict) -> Profile:
        try:
            ...
            orcid_record = _fetch_orcid_record(
                token_data["orcid"], token_data["access_token"]
            )
            email_addresses = [
                e["email"] for e in extract_email_addresses(orcid_record)
            ]
    ...
    def _create_profile(token_data: dict) -> Profile:
        ...
        profile = Profile(
            profiles.next_id(), Name(token_data["name"]), token_data["orcid"]
        )
    ...
    def _find_and_update_access_token(token_data: dict) -> OrcidToken:
        try:
            ...
            LOGGER.info(
                "ORCID token not found for %s. Creating Orcid Token.",
                token_data["orcid"],
            )
