...
from profiles.serializer.normalizer import (
    normalize,
...
def create_blueprint(profiles: Profiles) -> Blueprint:
    ...
    def _list() -> Response:
        ...
        profile_list = profiles.list(
            per_page, (page * per_page) - per_page, order == ORDER_DESC
        )
        ...
        response = make_response(
            json.dumps(
                {"total": total, "items": profile_list}, default=normalize_snippet
            )
        )
    ...
    def _get(profile_id: str) -> Response:
        ...
        if (
            ...
            in request.headers.get("x-consumer-groups", "").lower()
        ):
