...
class OrcidClient(object):
    ...
    def get_record(self, orcid: str, access_token: str) -> dict:
        ...
        try:
            response = self._get_request(
                "{}/{}/record".format(API_VERSION, orcid), access_token
            )
        except RequestException as exception:
            LOGGER.warning(
                "Failed to load ORCID record for %s (%s)", orcid, str(exception)
            )
            raise exception
    ...
    def set_webhook(self, orcid: str, webhook: str, access_token: str) -> None:
        ...
        try:
            self._request(
                "put",
                "{}/webhook/{}".format(orcid, quote(webhook, safe="")),
                access_token,
            )
        except RequestException as exception:
            LOGGER.warning(
                "Failed to set ORCID webhook %s for %s (%s)",
                webhook,
                orcid,
                str(exception),
            )
    ...
    def _request(
        self, method: str, path: str, access_token: str, headers: dict = None
    ) -> Response:
        if headers is None:
            headers = {}
